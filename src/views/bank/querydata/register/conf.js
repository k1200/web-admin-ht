import { fn_api__bank_querySysDepBooks } from '@/api/bank';
export default {
  name: 'BankRegister',
  data() {
    return {
  ...
  methods: {
    ...
    fn_handle__get_list(form = this.query_params) {
      this.loading = true;
      fn_api__bank_querySysDepBooks(this.fn_format__reqdata(form))
        .then(res => {
    ...
    // 格式化银行标识
    fn_formatter__tranFlag(row, column, cellValue) {
      let tranFlag = this.tranFlag_dict.find(item => item.value === cellValue);
      return tranFlag ? tranFlag.label : cellValue;
    }
  }
};
